...
def doctor_dashboard():
    ...
    st.markdown(f"<h3>Dr. {user['full_name'] or user['username']}</h3>", unsafe_allow_html=True)
    ...
    if choice == "Overview":
        ...
        if all_predictions:
            ...
            # Fix: risk_score is at index 17, not 14
            # Convert risk_scores to float to handle string values from database
            risk_scores = []
            for p in all_predictions:
                if p[17] is not None:
                    try:
                        risk_scores.append(float(p[17]))
                    except (ValueError, TypeError):
                        pass  # Skip invalid values
            
            avg_risk = np.mean(risk_scores) * 100 if risk_scores else 0
            
            # Safe high_risk_count calculation with type conversion
            high_risk_count = 0
            for p in all_predictions:
                if p[17] is not None:
                    try:
            ...
            for pred in all_predictions[:10]:  # Show last 10
                patient_name = pred[19] if len(pred) > 19 and pred[19] else pred[20]
                
                # Convert risk_score to float to handle string values from database
                try:
                    risk_score_float = float(pred[17]) if pred[17] is not None else 0
                except (ValueError, TypeError):
                    risk_score_float = 0
                ...
                recent_data.append({
                    ...
                    'BP': pred[5],
                    'Cholesterol': pred[6],
                    'Risk': f"{risk_score_float*100:.2f}%"
                })
        ...
        if all_predictions:
            ...
            for pred in all_predictions:
                patient_name = pred[19] if len(pred) > 19 and pred[19] else (pred[20] if len(pred) > 20 else 'Anonymous')
                ...
                risk_cat, _ = get_risk_category(risk_score_float)
                ...
                patient_data.append({
                    ...
                    'Cholesterol': pred[6],
                    'Max HR': pred[9],
                    'Risk Score': f"{risk_score_float*100:.2f}%",
                    'Category': risk_cat,
                    'Date': str(pred[18])[:10] if len(pred) > 18 and pred[18] else 'N/A'
                })
        ...
        if all_predictions:
            ...
            df = pd.DataFrame(all_predictions, columns=[
                'id', 'user_id', 'age', 'sex', 'cp', 'trestbps', 'chol', 'fbs', 'restecg',
                'thalach', 'exang', 'oldpeak', 'slope', 'ca', 'thal', 'chol_bp_ratio',
                'age_group', 'risk_score', 'date', 'name', 'username'
            ])
            
            # Convert risk_score to numeric type to handle string values from database
            ...
            with col1:
                ...
                # Drop NaN values from risk_score for histogram
                valid_risk_scores = df['risk_score'].dropna()
                if len(valid_risk_scores) > 0:
                    ax1.hist(valid_risk_scores*100, bins=20, color='#e63946', alpha=0.7, edgecolor='black')
            ...
            with col2:
                ...
                age_groups_data = df.dropna(subset=['age_group', 'risk_score']).copy()
                if len(age_groups_data) > 0:
                    age_groups = age_groups_data.groupby('age_group')['risk_score'].mean() * 100
                    ...
                    ax2.text(0.5, 0.5, 'No valid data available', ha='center', va='center', transform=ax2.transAxes)
                st.pyplot(fig2)
            ...
            if len(corr_df) > 0:
                ...
                st.info("Not enough valid data for correlation analysis")
            
            # Risk factors analysis
            st.subheader("⚠️ Risk Factor Analysis")
            # Drop NaN values before filtering
            df_valid = df.dropna(subset=['risk_score'])
            high_risk = df_valid[df_valid['risk_score'] > 0.6]
            low_risk = df_valid[df_valid['risk_score'] < 0.3]
            ...
            if len(high_risk) > 0 and len(low_risk) > 0:
                ...
                st.info("Insufficient data to compare risk groups")
